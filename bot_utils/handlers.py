...
async def start_with_category(call: types.CallbackQuery, state: FSMContext ):
    ...
    if user_data:
        ...
        await state.set_state(UserMessageState.answer_text)
        await call.message.answer(f" {film.name_text}")
...
async def send_questions(message:types.Message, state: FSMContext):
    tg_id = message("from")
    user_data = await redis_client.get_user_data()
    if user_data:
        answer = message.text
        user_film = await redis_client.get_user_data(tg_id)
        if answer == user_film("text"):
            await message.answer("ура вы угадали название фильма")
            await  redis_client.delete_user_film(tg_id)
            GuessedFilmManager().insert_guessed_film(tg_id,user_film["id"])
            film = get_random_film(tg_id,category=user_film["id"])
            redis_client.cache_user_film(tg_id,{"id":film.id, "text":film.name_text})
            await message.answer("Угадай следуший фильм")
            await message.answer(f"{film.emoji_text}")
        else:    
            await message.answer("Тест")
    else:
        text = "У вас нету игры. Напишите /start_game для начало новой игры "    
        await message.answer(text)
